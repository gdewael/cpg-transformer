...
print('Reading data ...')
dat = pd.read_csv(args.dataFile, sep='\t', header=None,skiprows=1)
dat = dat[dat.isnull().values.sum(-1) == 0]
...
for chrom_name in chroms:
    ...
    pos_chrom = dat_subset[1].values.astype('int32')
    label_chrom = (dat_subset[3].values>=0.5).astype('int8')
    
    count_meth_chrom = np.zeros(len(label_chrom))
    count_unmeth_chrom = np.zeros(len(label_chrom))
    
    
    
    subset_ind_C = np.in1d(pos_chrom, indices)
    ...
    pos_encoded[chrom_name] = pos_chrom[subset_ind_C].astype('int32')
